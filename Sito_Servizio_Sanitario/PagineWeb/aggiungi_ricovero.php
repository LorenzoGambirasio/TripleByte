...
function verificaPazienteEsistente($conn, $cssn) {
    ...
    $query->close();
    return [$pazienteTrovato, $nomeCompleto];
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($azione === 'aggiungi_paziente') {
        ...
        } elseif (empty($input_values['nuovo_nome']) ||
            empty($input_values['nuovo_cognome']) || empty($input_values['nuova_data_nascita']) || empty($input_values['nuovo_luogo_nascita']) || empty($input_values['nuovo_indirizzo'])) {
            $error_message = "I campi Nome, Cognome, Data di Nascita, Luogo di Nascita e Indirizzo sono obbligatori per il nuovo paziente.";
        } else {
            try {
                aggiungiNuovoPaziente(
                    $conn,
                    $input_values['nuovo_cssn'],
                    $input_values['nuovo_nome'],
                    $input_values['nuovo_cognome'],
                    $input_values['nuova_data_nascita'],
                    $input_values['nuovo_luogo_nascita'],
                    $input_values['nuovo_indirizzo']
                );
                $success_message = "Nuovo paziente " . htmlspecialchars($input_values['nuovo_nome']) . " " . htmlspecialchars($input_values['nuovo_cognome']) . 
                " aggiunto con successo! Ora puoi selezionarlo nel form ricovero.";
                $success_action = "paziente_aggiunto"; // Per JS
        ...
        $input_values['costo'] = trim($_POST['costo'] ?? '');
        ...
        if (!empty($input_values['durata']) && (!is_numeric($input_values['durata']) || $input_values['durata'] <= 0)) {
            $errors[] = "La durata deve essere un numero intero positivo maggiore di zero.";
        }
        if (!empty($input_values['costo']) && (!is_numeric($input_values['costo']) || $input_values['costo'] < 0)) {
        ...
        if (!empty($errors)) {
             ...
             list($pazienteEsiste, $nomePazienteDB) = verificaPazienteEsistente($conn, $input_values['paziente']);
             if (!$pazienteEsiste) {
                  $error_message = "Il CSSN del paziente ('" . htmlspecialchars($input_values['paziente']) . "') non esiste nel database.";
             } else {
    ...
    if ($haRicoveroAttivo) {
        $error_message = "Impossibile aggiungere il ricovero: il paziente " . htmlspecialchars($input_values['paziente_nome']) .
        " ha già un ricovero attivo (" . htmlspecialchars($infoRicoveroAttivo) .
        "). Completare prima il ricovero esistente.";
    } else {
        ...
        try {
            $insertRicoveroSql = "INSERT INTO Ricovero (codOspedale, cod, paziente, data, durata, motivo, costo, stato)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            $stmtRicovero->bind_param("ssssisdi",
                $input_values['cod_ospedale'], $cod_ricovero_generato, $input_values['paziente'],
                $input_values['data_ricovero'], $input_values['durata'], $input_values['motivo'],
                $input_values['costo'], $stato_ricovero
            );
...
$queryPazienti = "SELECT CSSN, nome, cognome FROM Cittadino WHERE deceduto != 1 OR deceduto IS NULL ORDER BY cognome, nome";
...
<input id="nuova_data_nascita" max="<?= date('Y-m-d') ?>" name="nuova_data_nascita" required="" type="date" value="<?= htmlspecialchars($input_values['nuova_data_nascita'] ?? '') ?>"/>
</div>
<div class="form-group">
...
<div class="paziente-item <?= ($paziente['CSSN'] == $selectedCssn) ? 'selected' : '' ?>" data-name="<?= htmlspecialchars($paziente['nome'] . ' ' .
$paziente['cognome']) ?>" data-value="<?= htmlspecialchars($paziente['CSSN']) ?>">
...
<input id="paziente" name="paziente" required="" type="hidden" value="<?= htmlspecialchars($selectedCssn ?? '') ?>"/>
</div>
</div>
<div class="form-group">
...
<?php $selectedOspedaleText = 'Seleziona Ospedale'; $selectedOspedaleValue = $input_values['cod_ospedale'] ?? null;
                            if ($selectedOspedaleValue && !empty($ospedali)) 
                            { foreach ($ospedali as $ospedale) { if ($ospedale['codice'] == $selectedOspedaleValue)
                            { $selectedOspedaleText = htmlspecialchars($ospedale['nome']); break; } } } echo $selectedOspedaleText; ?>
</div>
...
<input id="cod_ospedale" name="cod_ospedale" required="" type="hidden" value="<?= htmlspecialchars($selectedOspedaleValue ?? '') ?>"/>
</div>
</div>
<div class="form-group">
...
<input id="costo" name="costo" pattern="^\d+(\.\d{1,2})?$" placeholder="Es: 1500.50" required="" title="Inserire un numero positivo, es: 1500 o 1500.50" type="text"
value="<?= htmlspecialchars($input_values['costo'] ?? '') ?>"/>
...
<input 
  type="checkbox"
  id="patologia_<?= htmlspecialchars($patologia['cod']) ?>"
  name="patologie_selezionate[]"
  value="<?= htmlspecialchars($patologia['cod']) ?>"
  <?= $isChecked ?>
>
<label for="patologia_<?= htmlspecialchars($patologia['cod']) ?>">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (formAggiungiPaziente && nuovoCssnInput) {
        formAggiungiPaziente.addEventListener('submit', function(event) {
            ...
            if (cssnValue.length === 0) {
                isValid = false; errorMessage = 'Il campo CSSN è obbligatorio.';
            } else if (cssnValue.length !== 16) {
                isValid = false; errorMessage = 'Il CSSN deve essere esattamente di 16 caratteri.';
            } else if (!/^[A-Z0-9]{16}$/.test(cssnValue)) {
                isValid = false; errorMessage = 'Il CSSN non è valido (solo 16 caratteri alfanumerici maiuscoli).';
            }
    ...
    let apiValidationError = false;
    let apiValidationErrorMessage = "";

 
    if (luogoNascitaInput.value.trim() !== "" && luogoNascitaInput.dataset.selectedByUser !== "true") {
        apiValidationError = true;
        apiValidationErrorMessage = 'Per il Luogo di Nascita, seleziona un valore valido dalla lista dei suggerimenti.';
        luogoNascitaInput.focus();
    }
   if (!apiValidationError && indirizzoInput.value.trim() !== "" && indirizzoInput.dataset.selectedByUser !== "true") {
        apiValidationError = true;
        apiValidationErrorMessage = 'Per l\'Indirizzo, seleziona un valore valido dalla lista dei suggerimenti.';
    ...
    if (apiValidationError) {
        ...
        Swal.fire({
            icon: 'error',
            title: 'Selezione Invalida',
            text: apiValidationErrorMessage,
        ...
        return; 
            ...
            if (!isValid) {
                event.preventDefault();
                Swal.fire({ icon: 'error', title: 'Errore Validazione CSSN', text: errorMessage, confirmButtonColor: '#002080' });
                nuovoCssnInput.focus();
                if(nuovoCssnErrorSpan){ nuovoCssnErrorSpan.textContent = errorMessage; nuovoCssnErrorSpan.style.display = 'block'; }
            } else {
   ...
   function setupDropdown(toggleId, dropdownId, searchInputId, itemsSelector, isMultiSelect = false, hiddenInputId = null, valueDisplayId = null, badgesContainerId = null, countDisplayId = null) {
    ...
    if (costoInput) {
        costoInput.addEventListener('input', function() {
            ...
            if ((this.value.match(/\./g) || []).length > 1) { this.value = this.value.substring(0, this.value.lastIndexOf('.')); }
            const parts = this.value.split('.');
            if (parts[1] && parts[1].length > 2) { this.value = parts[0] + '.' + parts[1].substring(0, 2); }
        });
        costoInput.addEventListener('blur', function() {
            if (this.value && !isNaN(parseFloat(this.value)) && (this.value.match(/\./g) || []).length === 1 && this.value.endsWith('.')) {
                
                this.value = parseFloat(this.value).toFixed(2); 
            } else if (this.value && !isNaN(parseFloat(this.value)) && this.value.indexOf('.') === -1) {
                
                this.value = parseFloat(this.value).toFixed(2);
            } else if (this.value && !isNaN(parseFloat(this.value))) {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    async function aggiornaStatoCSSN() {
        ...
        if (provenienza === "Estero") {
            ...
            hiddenCssnField.value = cssnEstero;
    ...
    document.getElementById('formAggiungiPaziente').addEventListener('submit', function() {
        if (!cssnField.readOnly) {
            hiddenCssnField.value = cssnField.value;
        }
    });
});
